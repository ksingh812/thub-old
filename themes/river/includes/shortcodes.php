...
if (!function_exists('latest_post')) {
    function latest_post($atts, $content = null) {
        ...
        while ($q->have_posts()) : $q->the_post();
            ...
            $html .= '<span class="date">' . get_post_time('d', true, get_the_ID(), true) . ' </span>';
            $html .= '<span class="month">' . get_post_time('F', true, get_the_ID(), true) . ' </span>';
...
if (!function_exists('portfolio_list')) {
    ...
    function portfolio_list($atts, $content = null) {
        ...
        if (have_posts()) : while (have_posts()) : the_post();
                ...
                if ($lightbox == "yes") {
                    $html .= "<a class='preview' href='http://tickets.tickethub.co/ResultsGeneral.aspx?stype=0&kwds=" . get_the_title() . "'><i class='icon-ticket icon-2x'></i></a><a itemprop='url' class='preview' href='" . get_permalink() . "'><i class='icon-link icon-2x'></i></a>";
                }
                //$html .= "<a class='preview' href='" . get_permalink() . "'><i class='icon-link icon-2x'></i></a>";
                if ($type != "standard") {
