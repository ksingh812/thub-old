...
<?php if(isset($qode_options_river['header_height'])){ 
	if (!empty($qode_options_river['header_height'])) { ?>
	
	header_height = <?php echo $qode_options_river['header_height']; ?>;
	
	
<?php } } ?>
<?php if(isset($qode_options_river['header_height_scroll'])){ 
	if (!empty($qode_options_river['header_height_scroll'])) { ?>
	
	min_header_height = <?php echo $qode_options_river['header_height_scroll']; ?>;
	
<?php } } ?>
var logo_height = header_height;
<?php if(isset($qode_options_river['logo_image'])){ 
	if (!empty($qode_options_river['logo_image'])) {
        $logo_url_obj = parse_url($qode_options_river['logo_image']);
...
<?php
if($qode_options_river['enable_google_map'] != ""){
?>
...
if(isset($qode_options_river['google_maps_color'])){
	if (!empty($qode_options_river['google_maps_color']))
		$google_maps_color = $qode_options_river['google_maps_color'];
}
?>
  var mapStyles = [
    {
...
if(isset($qode_options_river['google_maps_scroll_wheel'])){
	if ($qode_options_river['google_maps_scroll_wheel'] == "yes")
		$google_maps_scroll_wheel = true;
}
$google_maps_zoom = 12;
if(isset($qode_options_river['google_maps_zoom'])){
	if (intval($qode_options_river['google_maps_zoom']) > 0)
		$google_maps_zoom = intval($qode_options_river['google_maps_zoom']);
}
?>
...
if(isset($qode_options_river['google_maps_style'])){
	if ($qode_options_river['google_maps_style'] == "no")
	...
	geocoder.geocode( { 'address': data}, function(results, status) {
		if (status === google.maps.GeocoderStatus.OK) {
			...
			var marker = new google.maps.Marker({
				...
				<?php if(isset($qode_options_river['google_maps_pin_image'])){ ?>
				icon:  '<?php echo $qode_options_river['google_maps_pin_image']; ?>',
				<?php } ?>
...
function showContactMap() {
	...
	if($j("#map_canvas").length > 0){
		initialize();
		codeAddress('<?php if(isset($qode_options_river['google_maps_address5'])) { echo $qode_options_river['google_maps_address5']; } ?>');
		codeAddress('<?php if(isset($qode_options_river['google_maps_address4'])) { echo $qode_options_river['google_maps_address4']; } ?>');
		codeAddress('<?php if(isset($qode_options_river['google_maps_address3'])) { echo $qode_options_river['google_maps_address3']; } ?>');
		codeAddress('<?php if(isset($qode_options_river['google_maps_address2'])) { echo $qode_options_river['google_maps_address2']; } ?>');
		codeAddress('<?php if(isset($qode_options_river['google_maps_address'])) { echo $qode_options_river['google_maps_address']; } ?>');
	}
}
...
<?php if($qode_options_river['parallax_speed'] != ''){ ?>
var parallax_speed = <?php echo $qode_options_river['parallax_speed']; ?>;
<?php }else{ ?>
...
if (isset($qode_options_river['internal_no_ajax_links'])) {
	foreach (explode(',', $qode_options_river['internal_no_ajax_links']) as $no_ajax_link) {
?>
